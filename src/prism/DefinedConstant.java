...
public class DefinedConstant
{
	...
	public void defineInt(String sl, String sh, String ss) throws PrismException
	{
		...
		if (sh == null) {
			...
			if (ih < il)
				throw new PrismException("Low value " + il + " for constant " + name + " is higher than the high value " + ih);
			if (ss == null) {
				// default step is 1
				is = 1;
			} else {
				...
				if (is == 0)
					throw new PrismException("Step value for constant " + name + " cannot be zero");
				if (is < 0)
					throw new PrismException("Step value for constant " + name + " must be positive");
				if (is > ih - il)
					throw new PrismException("Step value " + is + " for constant " + name + " is bigger than the difference between " + il + " and " + ih);
			}
		}
		// compute num steps
		numSteps = 0;
	...
	public void defineDouble(String sl, String sh, String ss) throws PrismException
	{
		...
		if (sh == null) {
			...
			if (dh < dl)
				throw new PrismException("Low value " + dl + " for constant " + name + " is higher than the high value " + dh);
			if (ss == null) {
				// default step is 1.0
				ds = 1.0;
			} else {
				...
				if (ds < 0)
					throw new PrismException("Step value for constant " + name + " must be positive");
				if (ds > dh - dl)
					throw new PrismException("Step value " + ds + " for constant " + name + " is bigger than the difference between " + dl + " and " + dh);
			}
		}
		// compute num steps
		numSteps = 0;
	...
	public void defineBoolean(String sl, String sh, String ss) throws PrismException
	{
		// parse value (low)
		if (sl.equals("true"))
			low = new Boolean(true);
		else if (sl.equals("false"))
			low = new Boolean(false);
		else
