...
public class SimulatorNetworkHandler extends Observable implements EntityResolver, SettingOwner, TreeNode, TableModel
{
	...
	public SimulatorNetworkHandler()
	{
		...
		this.sshCommand = new SingleLineStringSetting("ssh command", "/usr/bin/ssh2", "The location on the network cluster's filesystem of the ssh command",
				this, true);
		this.scpCommand = new SingleLineStringSetting("scp command", "/usr/bin/scp2", "The location on the network cluster's filesystem of the scp command",
				this, true);
	...
	public int addFileSystem(String name, String binaryDir, String resultsDir)
	{
	...
	{
		for (int i = 0; i < getNumHosts(); i++) {
			if (getHost(i).getHostState() == SSHHost.TESTING)
				return false;
		}
		...
		Thread t = new Thread(new Runnable()
		{
			...
			public void run()
			{
				try {
					...
					for (int i = 0; i < getNumHosts(); i++) {
						SSHHost sshh = getHost(i);
						if (sshh.getHostState() == SSHHost.STOPPED)
							sshh.doneSuccessfulStint();
					}
	...
	public int countOkayHosts()
	{
		...
		for (int i = 0; i < getNumHosts(); i++) {
			SSHHost sshh = getHost(i);
			if (sshh.getHostState() != SSHHost.ERROR && sshh.getHostState() != SSHHost.STOPPED)
				count++;
		}
		return count;
	}
	...
	private AbstractTableModel tableModel = new AbstractTableModel()
	{
		...
		public Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			case 1: {
				...
				case SSHHost.READY_OKAY:
					str = "Ready";
					break;
				case SSHHost.SENDING_FILES:
					str = "Sending Files...";
					break;
				case SSHHost.RUNNING:
					str = "Running...";
					break;
				case SSHHost.RETRIEVING_FILES:
					str = "Retrieving Files...";
					break;
				case SSHHost.ERROR:
					return "Error: " + host.getErrorMessage();
				case SSHHost.DONE:
					return "Done";
				case SSHHost.STOPPED:
					return "Stopped";
				default:
					return "";
				}
			...
			case 2:
				return "" + host.getTotalDone();
			case 3:
				return host.getProgressBar();
			case 4:
				return "" + host.getTotalStints();
			default:
				return "";
			}
		}
	...
	{
		for (int i = 0; i < getNumFileSystems(); i++) {
			if (index >= getFileSystem(i).getNumHosts())
				index -= getFileSystem(i).getNumHosts();
			else {
