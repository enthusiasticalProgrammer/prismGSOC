...
public class SSHHost extends Thread implements SettingOwner, TreeNode
{
	...
	public SSHHost(String hostName, String userName, FileSystem owner)
	{
		...
		this.hostName = new SingleLineStringSetting("host name", hostName,
		...
		this.userName = new SingleLineStringSetting("user name", userName,
				"This value must be a valid username for this host.  The host must also be setup for passwordless access using this username.", this, true);
	...
	public void sendFilesToFileSystem(File simBinary) throws PrismException
	{
		...
		try {
			...
			String[] parameters2 = { "-p", simBinary.getPath(), getUserName() + "@" + getHostName() + ":" + owner.getInputDir() };
	...
	{
		if (getHostState() != READY_OKAY && getHostState() != READY_UNKNOWN_STATUS)
			return; //must be ready
		...
		Thread stopThread = new Thread(new Runnable()
		{
			...
			public void run()
			{
				if (getHostState() == RUNNING) {
					...
					String[] parameters2 = { "-p", controlFile.getPath(),
	...
	class TestThread extends Thread
	{
		...
		public void run()
		{
			if (getHostState() != READY_UNKNOWN_STATUS && getHostState() != READY_OKAY && getHostState() != ERROR)
				return; //check we can do the test
			...
			try {
				...
				String[] parameters2 = { "-p", file.getPath(), getUserName() + "@" + getHostName() + ":" + owner.getInputDir() + "/" + testFile + ".txt" };
				// do the scp call
				SSHHandler.scp(getUserName(), getHostName(), parameters2);

				//Tidy up input directory again
				String[] parameters3 = { "-f", owner.getInputDir() + "/" + testFile + ".txt" };
				//do ssh call
				SSHHandler.ssh(getUserName(), getHostName(), "rm", parameters3);

				//Test output directory by creating a file in it, and copying it here`
				String[] parameters4 = { "test", owner.getOutputDir() + "/" + testFile + ".txt" };
	...
	class StintThread extends Thread
	{
		...
		public void run()
		{
			...
			try {
				...
				//transfer results
				String[] parameters2 = { "-p", getUserName() + "@" + getHostName() + ":" + owner.getOutputDir() + "/" + resultsFile + ".txt",
				...
				//tidy up the host
				String[] parameters3 = { "-f", owner.getOutputDir() + "/" + resultsFile + ".txt" };
				SSHHandler.ssh(getUserName(), getHostName(), "rm", parameters3);

			} catch (PrismException ee) {
				if (getHostState() == RUNNING)
					errorMessage = "Error when executing simulator on " + getHostName() + "\n" + ee.getMessage();
				else if (getHostState() == RETRIEVING_FILES)
					errorMessage = "Error when retrieving files from " + getHostName() + "\n" + ee.getMessage();
				else
					errorMessage = ee.getMessage();
				setState(ERROR);
				if (!stopping)
	...
	class StintFeedbackThread extends Thread
	{
		...
		public void run()
		{
			...
			try {
				while (getHostState() == RUNNING || getHostState() == SENDING_FILES) {
					...
					if (regular) {
						...
						String[] parameters = { "-p", getUserName() + "@" + getHostName() + ":" + owner.getOutputDir() + "/" + feedbackName,
					...
					if (results) {
						String[] parameters = { "-p", getUserName() + "@" + getHostName() + ":" + owner.getOutputDir() + "/" + feedbackResults,
			...
			String[] parameters = { "-f", owner.getOutputDir() + "/" + feedbackName }; //note if the tidy up doesn't work, it doesn't really matter.
			try {
				SSHHandler.ssh(getUserName(), getHostName(), "rm", parameters);
			} catch (PrismException e) {
				return;
			}
			String[] parameters2 = { "-f", owner.getOutputDir() + "/" + feedbackResults };
			try {
