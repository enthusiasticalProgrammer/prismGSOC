...
public class Modules2MTBDD
{
	...
	public Model translate() throws PrismException
	{
		...
		if (modelType == ModelType.DTMC) {
			model = new ProbModel(trans, start, stateRewards, transRewards, rewardStructNames, allDDRowVars, allDDColVars, ddVarNames, numModules, moduleNames,
					moduleDDRowVars, moduleDDColVars, numVars, varList, varDDRowVars, varDDColVars, constantValues);
		} else if (modelType == ModelType.MDP) {
			model = new NondetModel(trans, start, stateRewards, transRewards, rewardStructNames, allDDRowVars, allDDColVars, allDDSynchVars, allDDSchedVars,
					allDDChoiceVars, allDDNondetVars, ddVarNames, numModules, moduleNames, moduleDDRowVars, moduleDDColVars, numVars, varList, varDDRowVars,
					varDDColVars, constantValues);
		} else if (modelType == ModelType.CTMC) {
			model = new StochModel(trans, start, stateRewards, transRewards, rewardStructNames, allDDRowVars, allDDColVars, ddVarNames, numModules, moduleNames,
					moduleDDRowVars, moduleDDColVars, numVars, varList, varDDRowVars, varDDColVars, constantValues);
		}
	...
	private ComponentDDs translateModule(int m, Module module, String synch, int synchMin) throws PrismException
	{
		...
		for (l = 0; l < numCommands; l++) {
			...
			if (synch == "") {
				...
				if (command.getSynch().equals(synch))
					match = true;
			}
			// if so translate
			if (match) {
	...
	private void computeRewards(SystemDDs sysDDs) throws PrismException
	{
		...
		for (j = 0; j < numRewardStructs; j++) {
			...
			for (i = 0; i < numSynchs; i++)
				sysDDs.synchs[i].rewards[j] = JDD.Constant(0);
		}
	...
	private void doSymmetry(Model model) throws PrismException
	{
		...
		for (k = 0; k < numRewardStructs; k++) {
			...
			mainLog.println(
					JDD.GetInfoString(transRewards[k], (modelType == ModelType.MDP) ? (allDDRowVars.n() * 2 + allDDNondetVars.n()) : (allDDRowVars.n() * 2)));
			JDD.Ref(symm);
		...
		for (i = numSymmModules; i > 1 && !done; i--) {
			...
			for (j = 0; j < i - 1; j++) {
				...
				for (k = 0; k < numRewardStructs; k++) {
					...
					JDD.Ref(nonSymms[j]);
					JDD.Ref(tmp);
					transRewards[k] = JDD.ITE(JDD.PermuteVariables(nonSymms[j], allDDRowVars, allDDColVars), JDD.Constant(0),
							JDD.Apply(JDD.PLUS, transRewards[k], tmp));
					//mainLog.print("transrew["+k+"] (symm): ");
					//mainLog.println(JDD.GetInfoString(transRewards[k], (type==ModulesFile.NONDETERMINISTIC)?(allDDRowVars.n()*2+allDDNondetVars.n()):(allDDRowVars.n()*2)));
					JDD.Deref(tmp);
				}
