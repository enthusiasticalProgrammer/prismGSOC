	...
	public static String convert(Node node, ArrayList<String> renameFrom, ArrayList<String> renameTo) throws PrismException
	{
		...
		case Node.ELEMENT_NODE:
			...
			// Identifier
				...
				s = node.getFirstChild().getNodeValue().trim();
				if (renameFrom != null)
					if (renameFrom.contains(s))
						s = renameTo.get(renameFrom.indexOf(s));
				return s;
			}
			...
			// Apply operator
				...
				if (apply.equals("minus")) {
					if (n != 2)
						throw new PrismException("MathML apply minus operations has " + n + " operands");
				...
				if (apply.equals("divide")) {
					if (n != 2)
						throw new PrismException("MathML apply divide operations has " + n + " operands");
					s = "(" + translatedChildren.get(0) + "/" + translatedChildren.get(1) + ")";
					return s;
				}
				...
				if (apply.equals("power")) {
					if (n != 2)
						throw new PrismException("MathML apply power operations has " + n + " operands");
