...
public class SafraTreeNode implements Iterable<SafraTreeNode>
{
	 ...
	 */
	public void setLabeling(MyBitSet labeling)
	{
		_labeling = labeling;
	}
	...
	public boolean equals(SafraTreeNode other)
	{
		...
		if (!_labeling.equals(other._labeling)) {
			return false;
		}
		if (!null_or_eq(_parent, other._parent)) {
			return false;
		}
		if (!null_or_eq(_olderBrother, other._olderBrother)) {
			return false;
		}
		if (!null_or_eq(_youngerBrother, other._youngerBrother)) {
			return false;
		}
		if (!null_or_eq(_oldestChild, other._oldestChild)) {
			return false;
		}
		if (!null_or_eq(_youngestChild, other._youngestChild)) {
			return false;
		}
	...
	public boolean lessThan(SafraTreeNode other)
	{
		...
		if (_childCount < other._childCount)
			return true;
		if (_labeling.compareTo(other._labeling) < 0)
			return true;
