...
public class ModuleModel extends SelectionModel implements Observer
{
		...
		addState.addActionListener(new java.awt.event.ActionListener()
		{
			...
			public void actionPerformed(java.awt.event.ActionEvent e)
			{
				if (!drawingProbTrans) {
					deSelectAll();
					if (snap)
						addState(snapIt(popupX), snapIt(popupY));
					else
						addState(popupX, popupY);
					setChanged();
					notifyObservers(null);
				}
			}
	...
	public int addProbTransition(State from, State to, Nail aNail, String probExpression)
	{
		modified = true;
		if (aNail != null) {
			ArrayList<Nail> keepNails = new ArrayList<>();
			keepNails.add(aNail);
				...
				for (int i = 0; i < selection.transitions.length; i++) {
					...
					//if(!trsel.prob)
					{
						...
						for (int j = 0; j < originalIndices.length; j++) {
							if (originalIndices[j] == trsel.from.id)
								indexFrom = newIndices[j];
							if (originalIndices[j] == trsel.to.id)
								indexTo = newIndices[j];
						}
	...
	public void deleteSelected()
	{
		...
		ArrayList<Transition> removeTr = new ArrayList<>();
		ArrayList<State> removeSt = new ArrayList<>();
		for (int i = 0; i < numTransitions; i++) {
		...
		ArrayList<Nail> removeNa = new ArrayList<>();
		for (int i = 0; i < numNails; i++) {
	...
	public void finaliseTempBranch()
	{
		...
		while (contin) {
			...
			ArrayList<State> sta = new ArrayList<>();
			sta.add(toState);
			int counter = 1;
			...
			if (counter == 1) {
				if (!(toState == tempProbFrom)) // if this state does not go back to the original state
					...
					addProbTransition(getState(dec), toState, new Nail(((getState(dec).getX() + toState.getX()) / 2) + 30,
							((getState(dec).getY() + toState.getY()) / 2) + 30, getState(dec), toState), "1/" + branchCount);
	...
	public void zoomInOn(Rectangle2D area)
	{
		...
		if (area.getWidth() < 30 || area.getHeight() < 30)
			return;
	...
	public void processDoubleClick(double x, double y)
	{
		...
		if (mode == EDIT) {
			...
			for (int i = 0; i < numTransitions; i++) {
				Transition tr = (Transition) transitions.get(i);
				if (tr.intersects(mouseBox) || tr.getGuardLabel().intersects(mouseBox) || tr.getSyncLabel().intersects(mouseBox)
						|| tr.getAssignmentLabel().intersects(mouseBox) || tr.getProbabilityLabel().intersects(mouseBox)) {
			...
			if (!drawingProbTrans && !drawingExtraBranches) {
				deSelectAll();
				if (snap)
					addState(snapIt(actualX) - 15, snapIt(actualY) - 15);
				else
					addState(actualX - 15, actualY - 15);
			}
		...
		if (mode == ZOOM) {
			...
			if (drawingProbTrans) {
				...
				if (lastProbNail == null) {
					...
					Nail theNail;
					if (snap)
						theNail = new Nail(snapIt(actualX), snapIt(actualY), tempProbFrom, null);
					else
						theNail = new Nail(actualX, actualY, tempProbFrom, null);
					...
					Nail theNail;
					if (snap)
						theNail = new Nail(snapIt(actualX), snapIt(actualY), lastProbNail, null);
					else
						theNail = new Nail(actualX, actualY, lastProbNail, null);
		...
		if (mode == ZOOM) {
			...
			} else // menus
			{
				...
				while (!(comp instanceof JSeparator)) {
					if (modulePopup.getComponentCount() > 0)
						modulePopup.remove(0);
	...
	public void processMouseUp(double x, double y)
	{
		...
		if (mode == ZOOM) {
			...
			if (drawingProbTrans)
				...
				Rectangle2D.Double selection = new Rectangle2D.Double(selX, selY, selWidth, selHeight);
				...
				if (snap) {
					for (int i = 0; i < numStates; i++) {
						...
						if (sta.isSelected()) {
							...
							sta.setPosition(snapIt(sta.getX() + offset) - offset, snapIt(sta.getY() + offset) - offset);
							sta.stopMoving();
						}
					}
					for (int i = 0; i < numNails; i++) {
						Nail na = (Nail) theNails.get(i);
						if (na.isSelected()) {
							na.setPosition(snapIt(na.getX()), snapIt(na.getY()));
							na.stopMoving();
						}
					}
				}
	...
	public void processDrag()
	{
		...
		if (mode == ZOOM) {
			...
			if (isDrawingExtraBranches()) {
				...
				if (lastTranDrawX < minX)
					minX = lastTranDrawX - 10;
				if (lastTranDrawX > maxX)
					maxX = lastTranDrawX + 10;
				if (lastTranDrawY < minY)
					minY = lastTranDrawY - 10;
				if (lastTranDrawY > maxY)
					maxY = lastTranDrawY + 10;
		...
		if (mode == EDIT) {
			...
			if (!isMoving && !isDrawingProbTrans() && !isSelecting && !isZoomSelecting) {
				...
				for (int i = 0; i < numTransitions; i++) {
					...
					if (tr instanceof ProbTransition) {
						if (((ProbTransition) tr).getProbabilityLabel().intersects(mouseBox)) {
							isOverMovable = true;
							setOverTransition(false);
							return;
						}
	...
	public void orderStates()
	{
		...
		ArrayList<Object> notNormal = new ArrayList<>();
		for (int i = 0; i < theStates.size(); i++) {
