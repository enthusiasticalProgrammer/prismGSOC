...
public class ModulesPanel extends JPanel implements MouseListener, ComponentListener
{
			...
			if (e.getComponent() == this) {
				...
				for (int i = 0; i < containers.size(); i++) {
					...
					if (gme.getInternalFrame().isMaximum()) {
						dp.setPreferredSize(new Dimension(scr.getViewport().getWidth(), scr.getViewport().getHeight()));
						gme.getInternalFrame().setPreferredSize(new Dimension(dp.getWidth(), dp.getHeight()));
						if (gme.getInternalFrame() == e.getComponent())
							a_maximiseAll();
						break;
					} else {
						if (gme.getInternalFrame() == e.getComponent())
	...
	public void a_tile()
	{
		...
		for (int i = 0; i < containers.size(); i++) {
			GraphicModuleContainer gmc = (GraphicModuleContainer) containers.get(i);
			if (!gmc.isDocked() || !gmc.getInternalFrame().isVisible())
				continue;
			...
			if (gmc.getInternalFrame().getHeight() > maxHeightSoFar)
				maxHeightSoFar = gmc.getInternalFrame().getHeight();
	...
	public void a_cascade()
	{
		...
		for (int i = 0; i < containers.size(); i++) {
			...
			currX += border;
			currY += border;
			if (currY + gmc.getInternalFrame().getHeight() > maxHeightSoFar)
				maxHeightSoFar = currY + gmc.getInternalFrame().getHeight();
			gmc.setSelected();

		}
		//reselect the original
		if (previousSelection > -1)
			getModuleContainer(previousSelection).setSelected();
