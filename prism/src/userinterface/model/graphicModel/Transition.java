...
public class Transition implements PropertyOwner
{
	...
	public Transition(State from, State to, Prism pr)
	{
		...
		sync = new StringLabel("", this, "Sync", false,
				"A string describing the name of the synchronisation action for this transition.  If this is left blank, the transition is performed asynchronously.");
		sync.setOffsetX(-10);
		sync.setOffsetY(0);
	...
	public double getMinX()
	{
		...
		if (guard.getMinX() < min)
			min = guard.getMinX();
		if (sync.getMinX() < min)
			min = sync.getMinX();
		if (probability.getMinX() < min)
			min = probability.getMinX();
		if (assignment.getMinX() < min)
			min = assignment.getMinX();
	...
	public double getMaxX()
	{
		...
		if (guard.getMaxX() > max)
			max = guard.getMaxX();
		if (sync.getMaxX() > max)
			max = sync.getMaxX();
		if (probability.getMaxX() > max)
			max = probability.getMaxX();
		if (assignment.getMaxX() > max)
			max = assignment.getMaxX();
	...
	public double getMinY()
	{
		...
		if (guard.getMinY() < min)
			min = guard.getMinY();
		if (sync.getMinY() < min)
			min = sync.getMinY();
		if (probability.getMinY() < min)
			min = probability.getMinY();
		if (assignment.getMinY() < min)
			min = assignment.getMinY();
	...
	public double getMaxY()
	{
		...
		if (guard.getMaxY() > max)
			max = guard.getMaxY();
		if (sync.getMaxY() > max)
			max = sync.getMaxY();
		if (probability.getMaxY() > max)
			max = probability.getMaxY();
		if (assignment.getMaxY() > max)
			max = assignment.getMaxY();
	...
	public int compareTo(Object o)
	{
		if (o instanceof PropertyOwner) {
			...
			if (getUniquePropertyID() < po.getUniquePropertyID())
				return -1;
			else if (getUniquePropertyID() > po.getUniquePropertyID())
				return 1;
			else
				return 0;
		} else
	...
	{
		if (to instanceof Decision)
			return prism.PropertyConstants.BRANCHTRANSITION;
		else
