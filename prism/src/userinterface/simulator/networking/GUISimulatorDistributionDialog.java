...
public class GUISimulatorDistributionDialog extends javax.swing.JDialog implements Observer
{
	...
	{//GEN-HEADEREND:event_doneButtonActionPerformed
		if (network.countIterationsDone() < info.getNumSamples()) {
			...
			int selectionNo = -1;
			selectionNo = JOptionPane.showOptionDialog(this,
					"Not enough iterations have been performed.\nDo you want to use the available data?\n(This may decrease the accuracy of the results)",
					"Question", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, selection, selection[0]);
			switch (selectionNo) {
			case 0:
				hide();
				break;
	...
	{//GEN-HEADEREND:event_startButtonActionPerformed
		...
		if (!isExperiment) {
			try {
				...
				network.doNetworking(info.getNumSamples(), info.getMaxPathLength(), binfile, feedbackIterationsProgressRadio.isSelected(),
						feedbackResultsProgressRadio.isSelected());
			...
			try {
				...
				network.doNetworking(info.getNumSamples(), info.getMaxPathLength(), binfile, feedbackIterationsProgressRadio.isSelected(),
						feedbackResultsProgressRadio.isSelected(), undefinedConstants.getMFConstantValues(), propertyValues, expr.getResults());
	...
	public void update(Observable o, Object arg)
	{
		...
		if (propResultsModel != null)
			propResultsModel.updateTable();
	}
