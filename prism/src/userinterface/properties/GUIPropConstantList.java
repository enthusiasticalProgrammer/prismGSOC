...
public class GUIPropConstantList extends JTable
{
	...
	{
		if (theModel.error != null)
			return false;
		int i, n;
		n = theModel.getNumConstants();
		for (i = 0; i < n; i++) {
			if (!theModel.getConstant(i).isParseable())
				return false;
		}
		return true;
	}
	...
	class PropConstantModel extends AbstractTableModel
	{
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex)
		{
			...
			case 1: {
				...
				if (((String) aValue).equals("double"))
					newType = TypeDouble.getInstance();
				else if (((String) aValue).equals("bool"))
					newType = TypeBool.getInstance();
