...
public class ConstantLine extends javax.swing.JPanel
{
	...
	public void setSingleValue(String str)
	{
		if (type instanceof TypeBool) {
			if (str != null)
				boolSingleValueCombo.setSelectedIndex(str.equals("true") ? 0 : 1);
			else
				boolSingleValueCombo.setSelectedIndex(0); // default
		} else {
			if (str != null)
				singleValueField.setText(str);
			else
				singleValueField.setText(SINGLE_DEFAULT);
		}
	}
	...
	public void checkValid() throws PrismException
	{
		...
		if (type instanceof TypeBool) {
			...
			try {
				s = singleValueField.getText();
				Integer.parseInt(s);
				if (isRange()) {
					s = startValueField.getText();
					Integer.parseInt(s);
					s = endValueField.getText();
					Integer.parseInt(s);
					s = stepValueField.getText();
					Integer.parseInt(s);
				}
