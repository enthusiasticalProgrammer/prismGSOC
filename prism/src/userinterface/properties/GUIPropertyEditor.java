...
public class GUIPropertyEditor extends javax.swing.JDialog implements ActionListener, KeyListener
{
	...
	private void addString(String str, int position, int relativeCursorPosition)
	{
		...
		if (position == START)
			...
			curs = propertyText.getText().length();
		else
			curs = propertyText.getCaretPosition(); // must be curs
		...
		if (e.getSource() == trueButton) {
			...
			if (propertyText.getSelectedText() == null) {
				...
				for (int i = 0; i < end - start; i++) {
					if (propertyText.getSelectedText().charAt(i) == '(')
						open++;
					else if (propertyText.getSelectedText().charAt(i) == ')')
						closed++;
					else if (propertyText.getSelectedText().charAt(i) == '[')
						opensq++;
					else if (propertyText.getSelectedText().charAt(i) == ']')
				...
				if ((open == closed) && (opensq == closedsq))
					shouldDo = true;
				if (shouldDo) {
	...
	{//GEN-HEADEREND:event_okayButtonActionPerformed
		...
		try {
			...
			if (ff.getConstantList().size() != noConstants)
				throw new PrismException("Contains constants");
			if (ff.getLabelList().size() != noLabels)
		...
		// catch and deal with exceptions
			...
			case GUIMultiProperties.WARN_INVALID_PROPS: {
				...
				choice = props.optionPane("Error: " + ex.getMessage() + "\nAre you sure you want to continue?", "Question", JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE, choices, choices[0]);
				valid = (choice == 0);
				break;
			}
