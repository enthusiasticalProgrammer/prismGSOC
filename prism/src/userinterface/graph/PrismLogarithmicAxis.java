...
public class PrismLogarithmicAxis extends ValueAxis
{
	...
	protected List refreshTicksHorizontal(Graphics2D g2, Rectangle2D dataArea, RectangleEdge edge)
	{
		...
		while (current <= end) {
			...
			if (range.contains(v)) {
				ticks.add(new NumberTick(new Double(v), createTickLabel(v), TextAnchor.TOP_CENTER, TextAnchor.CENTER, 0.0));
			}
	...
	protected List refreshTicksVertical(Graphics2D g2, Rectangle2D dataArea, RectangleEdge edge)
	{
		...
		while (current <= end) {
			...
			double v = calculateValue(current);
			if (range.contains(v)) {
				ticks.add(new NumberTick(new Double(v), createTickLabel(v), TextAnchor.CENTER_RIGHT, TextAnchor.CENTER, 0.0));
			}
			// add minor ticks (for gridlines)
			double next = Math.pow(this.base, current + this.tickUnit.getSize());
			for (int i = 1; i < this.minorTickCount; i++) {
