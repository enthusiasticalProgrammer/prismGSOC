...
public class SeriesSettings extends Observable implements SettingOwner
{
	...
	public static final Shape[] DEFAULT_SHAPE_SEQUENCE = { DefaultDrawingSupplier.DEFAULT_SHAPE_SEQUENCE[1], // CIRCLE
	...
	protected static final Shape[] SHAPES = { new Ellipse2D.Double(-2, -2, 4, 4), new Rectangle2D.Double(-2, -2, 4, 4), new Polygon(triPointsX, triPointsY, 3),
			new Rectangle2D.Double(-2, -1, 4, 2), new Rectangle2D.Double(-1, -2, 2, 4), new Rectangle2D.Double(0, 0, 0, 0) // NONE    	
	};

	protected static final Shape[] DEFAULT_SHAPES = { SHAPES[CIRCLE], SHAPES[SQUARE], SHAPES[TRIANGLE], SHAPES[RECTANGLE_H], SHAPES[RECTANGLE_V] };

	protected static final Paint[] DEFAULT_PAINTS = { Color.blue, new Color(0, 127, 0), Color.red, new Color(0, 191, 191), new Color(191, 0, 191),
			new Color(191, 191, 0), new Color(0.6f, 0.2f, 0f) };
	...
	public void updateSeries()
	{
		...
		synchronized (graph.getSeriesLock()) {
			...
			if (seriesIndex >= 0) {
				/* Set series colour. */
				if (renderer.getSeriesPaint(seriesIndex) == null || !renderer.getSeriesPaint(seriesIndex).equals(seriesColour)) {
				...
				} else if (lineStyle.getCurrentIndex() == DASHED) // Just dash
				{
					...
					BasicStroke newStroke = new BasicStroke((float) lineWidth.getDoubleValue(), BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10.0f, dash,
							1.0f);
					renderer.setSeriesStroke(seriesIndex, newStroke);
				} else // Funny dash
				{
			...
			icon = new GraphSeriesIcon(renderer.getSeriesShape(seriesIndex), renderer.getSeriesStroke(seriesIndex), seriesColour.getColorValue(),
					showLines.getBooleanValue(), showPoints.getBooleanValue());
