...
public final class FoxGlynn
{
	...
	private final void run() throws PrismException
	{
		if (q_tmax == 0.0) {
		...
		} else if (q_tmax < 400) { //here naive approach should have better performance than Fox Glynn
		...
		} else { //use actual Fox Glynn for q_tmax>400
			if (accuracy < 1e-10) {
			...
			//run FINDER to get left, right and weight[m]
			{
				...
				for (k = lower_k_1; k <= upper_k_1; k = (k == lower_k_1) ? k + 4 : k + 1) {
