...
public class PTAParser implements PTAParserConstants
{
	...
	static class astTransition
	{
		...
		public astEdge addEdge(double prob, String dest)
		{
			astEdge e = new astEdge(prob, dest);
			edges.add(e);
			return e;
		}
	...
	static class astEdge
	{
		...
		// Methods
		public astEdge(double prob, String dest)
		{
			this.prob = prob;
			this.dest = dest;
			resets = new HashMap<>();
		...
		public void createDataStructures(astPTA pta, Transition trans)
		{
			...
			if (d == -1) {
				System.err.println("Error: Location \u005c"" + dest + "\u005c" does not exist");
				System.exit(1);
			}
			Edge edge = trans.addEdge(prob, d);
			for (Map.Entry<Integer, Integer> e : resets.entrySet())
				edge.addReset(e.getKey(), e.getValue());
		}
	}
	...
	static final public void Constraint(astPTA pta, LinkedHashSet<Constraint> constrs) throws ParseException
	{
		...
		case REG_INT: {
			...
			case PTAParserConstants.LT:
				constrs.add(Constraint.buildLt(clock1, val));
				break;
			case PTAParserConstants.LE:
				constrs.add(Constraint.buildLeq(clock1, val));
				break;
			case PTAParserConstants.GT:
				constrs.add(Constraint.buildGt(clock1, val));
				break;
			case PTAParserConstants.GE:
				constrs.add(Constraint.buildGeq(clock1, val));
				break;
		...
		case REG_IDENT: {
			...
			switch (t.kind) {
			case PTAParserConstants.LT:
				constrs.add(Constraint.buildLt(clock1, clock2));
				break;
			default:
